...
/*   Updated: 2026/05/10 18:21:13 by moerrais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void clean_queue_pro(t_queue *queue)
{
    ...
    i = 0;
    pthread_mutex_destroy(&queue->mutex_queue_priority);
...
bool ft_init_queue(t_simulation *sim)
{
    ...
    if (!queue)
        return (false);
    if (pthread_mutex_init(&queue->mutex_queue_priority, NULL) != 0)
