...
/*   Updated: 2026/04/15 11:40:10 by moerrais         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	compile(config->time_to_compile);
	compile(config->time_to_debug);
	compile(config->time_to_refactor);
	return NULL;
}
void	*create_in_threads(t_config config, t_threads_id *id_ths)
{
	...
	id_ths->thread_id = (pthread_t *)malloc((size_t) sizeof(pthread_t) * config.number_of_coders);
	if (!id_ths->thread_id)
	{
		return (NULL);
	}
